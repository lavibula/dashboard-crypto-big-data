...
async function fetchData(crypto) {
  try {
    const endpoint = `http://localhost:3001/api/${crypto}_pre?crypto=BTC`;
...
async function fetchIndicatorTable(indicator) {
  ...
  switch (indicator) {
    case "MACD":
      url = "http://localhost:3001/api/macd_table";
      break;
    case "RSI":
      url = "http://localhost:3001/api/rsi_table";
      break;
    case "SMA":
      url = "http://localhost:3001/api/sma_table";
      break;
    case "EMA":
      url = "http://localhost:3001/api/ema_table";
      break;
  }
...
async function fetchEMAData() {
  const response = await fetch("http://localhost:3001/api/ema_pre?crypto=BTC");
  const data = await response.json();
